...
select orders.customer_id,Orders.order_id, orders.order_date, customers.first_name, customers.last_name, customers.email
from orders
...
from products
inner join Order_Items on products.product_id = Order_Items.order_id
inner join orderson Order_Items.order_id = orders.order_id
...
from customers
inner join orderson customers.customer_id = orders.customer_id
inner join order_itemson orders.order_id = order_items.order_id
inner join productson order_items.product_id = products.product_id
GROUP BY 
